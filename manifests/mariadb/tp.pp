...
# Use equivalent class tp_profile::mariadb in the tp_profile module as replacement
# psick::mariadb::tp
#
...
class psick::mariadb::tp (
  ...
  if $manage {
    notify { 'Deprecated profile psick::mariadb::tp':
      message => 'This profile has been deprecated. It will be removed on version 1.0.0 of psick module. Use equivalent class tp_profile::mariadb in the tp_profile module as replacement',
    }
    if !$::psick::noop_mode and $no_noop {
      info('Forced no-noop mode in psick::mariadb::tp')
      noop(false)
    }
