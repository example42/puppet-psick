...
define psick::mariadb::grant (
  ...
  $grant_file = $dbname ? {
    /^(\*|%)$/ => "mariadbgrant${grant_file_host}-${user}-${nice_host}-all.sql",
    default    => "mariadbgrant${grant_file_host}-${user}-${nice_host}-${dbname}.sql",
  }
  ...
  if $db_init_query_file != '' and $create_db == true {
    psick::mariadb::sqlfile { "db_init_query_file-${nice_host}-${dbname}":
      file      => $db_init_query_file,
      user      => $remote_user,
      password  => $remote_password,
      db        => $dbname,
      host      => $manage_remote_host,
      subscribe => Exec["mariadbgrant-${user}-${nice_host}-${dbname}"],
    }
  }
}
