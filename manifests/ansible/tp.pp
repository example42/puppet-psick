...
# Use equivalent class tp_profile::ansible in the tp_profile module as replacement
# psick::ansible::tp
#
# @summary This psick profile manages ansible with Tiny Puppet (tp)
#
# @example Include it to install ansible
#   include psick::ansible::tp
#
# @example Include in PSICK via hiera (yaml)
#   psick::profiles::linux_classes:
#     ansible: psick::ansible::tp
#
# @example Manage extra configs via hiera (yaml) with templates based on custom options
#   psick::ansible::tp::ensure: present
#   psick::ansible::tp::resources_hash:
#     tp::conf:
#       ansible:
...
#   psick::ansible::tp::auto_conf: 'default'
#
...
#   Check psick::ansible::tp:resources_auto_conf_hash in data/$auto_conf/*.yaml for
...
#   Check psick::ansible::tp:options_auto_conf_hash in data/$auto_conf/*.yaml for
#   the auto_conf defaults.
...
class psick::ansible::tp (
  ...
  if $manage {
    notify { 'Deprecated profile psick::ansible::tp':
      message => 'This profile has been deprecated. It will be removed on version 1.0.0 of psick module. Use equivalent class tp_profile::ansible in the tp_profile module as replacement',
    }
    if !$::psick::noop_mode and $no_noop {
      info('Forced no-noop mode in psick::ansible::tp')
      noop(false)
    }
