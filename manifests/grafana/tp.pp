...
#   include psick::grafana::tp
#
# @example Include in PSICK via hiera (yaml)
#   psick::profiles::linux_classes:
#     grafana: psick::grafana::tp
#
# @example Manage extra configs via hiera (yaml) with templates based on custom options
#   psick::grafana::tp::ensure: present
#   psick::grafana::tp::resources_hash:
#     tp::conf:
#       grafana:
#         epp: profile/grafana/grafana.conf.epp
#       grafana::dot.conf:
#         epp: profile/grafana/dot.conf.epp
#         base_dir: conf
#   psick::grafana::tp::options_hash:
#     key: value
#
...
#   psick::grafana::tp::auto_conf: 'default'
#
# @example Set no-noop mode and enforce changes even if noop is set for the agent
#   psick::grafana::tp::no_noop: true
#
# @param manage If to actually manage any resource in this profile or not
# @param ensure If to install or remove grafana. Valid values are present, absent, latest
#   or any version string, matching the expected grafana package version.
# @param resources_hash An hash of tp::conf and tp::dir resources for grafana.
...
#   Check psick::grafana::tp:resources_auto_conf_hash in data/$auto_conf/*.yaml for
...
#   Check psick::grafana::tp:options_auto_conf_hash in data/$auto_conf/*.yaml for
#   the auto_conf defaults.
# @param settings_hash An hash of tp settings to override default grafana file
...
# @param auto_prereq If to automatically install eventual dependencies for grafana.
#   Set to false if you have problems with duplicated resources, being sure that you
#   manage the prerequistes to install grafana (other packages, repos or tp installs).
