...
# Use equivalent class tp_profile::openswan in the tp_profile module as replacement
# psick::openswan::tp
#
# @summary This psick profile manages openswan with Tiny Puppet (tp)
#
# @example Include it to install openswan
#   include psick::openswan::tp
#
# @example Include in PSICK via hiera (yaml)
#   psick::profiles::linux_classes:
#     openswan: psick::openswan::tp
#
...
#   psick::openswan::tp::auto_conf: 'default'
#
# @param manage If to actually manage any resource in this profile or not
# @param ensure If to install or remove openswan. Valid values are present, absent, latest
#   or any version string, matching the expected openswan package version.
# @param resources_hash An hash of tp::conf and tp::dir resources for openswan.
...
#   Check psick::openswan::tp:resources_auto_conf_hash in data/$auto_conf/*.yaml for
...
#   Check psick::openswan::tp:options_auto_conf_hash in data/$auto_conf/*.yaml for
#   the auto_conf defaults.
# @param settings_hash An hash of tp settings to override default openswan file
...
# @param auto_prereq If to automatically install eventual dependencies for openswan.
#   Set to false if you have problems with duplicated resources, being sure that you
#   manage the prerequistes to install openswan (other packages, repos or tp installs).
...
class psick::openswan::tp (
  ...
  if $manage {
    notify { 'Deprecated profile psick::openswan::tp':
      message => 'This profile has been deprecated. It will be removed on version 1.0.0 of psick module. Use equivalent class tp_profile::openswan in the tp_profile module as replacement',
    }
    if !$::psick::noop_mode and $no_noop {
      info('Forced no-noop mode in psick::openswan::tp')
      noop(false)
    }
